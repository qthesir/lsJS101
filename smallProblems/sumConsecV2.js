...
function getArray() {
  let input = readline.question(
    "Please enter a list of items separated by commas: "
  );
  input = input.split(",");
  let result = input.map((item) => Number(item.trim()));
  return result;
}
...
// Further exploration: What if the input was an array of integers instead of just a
// single integer? How would your computeSum and computeProduct functions change? Given
// that the input is an array, how might you make use of the Array.prototype.reduce() method?
...
if (operation === "s") {
  result = computeSum(array);
  console.log(`\n The sum of the integers in ${array} is ${result}.`);
} else if (operation === "p") {
  result = computeProduct(array);
  console.log(`\n The product of the integers in ${array} is ${result}.`);
} else {
  console.log("Oops. Something went wrong.");
}

// Reflection: If a step in your algorithm is sufficiently abstract, then make it into a 
// helper function. Helper functions also improve readability of the code. 